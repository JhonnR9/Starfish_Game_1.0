...
open class BaseActor(x: Float, y: Float, stage: Stage) : Group() {
    ...
    companion object {
        ...
        fun getList(stage: Stage, className: String): ArrayList<BaseActor> {
            ...
            try {
                val theClass: Class<*> = Class.forName("me.jhonn.game.entities.$className")
    ...
    fun isWithinDistance(distance: Float, other: BaseActor): Boolean {
        val poly0 = this.boundaryPolygon
        val scaleX = (width + 2 * distance)/width
        val scaleY = (height + 2 * distance)/height
        poly0?.setScale(scaleX,scaleY)

        val poly1 = other.boundaryPolygon
        if (!poly0?.boundingRectangle?.overlaps(poly1?.boundingRectangle)!!) return false

        return Intersector.overlapConvexPolygons( poly0, poly1 )
