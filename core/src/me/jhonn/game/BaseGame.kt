...
abstract class BaseGame : Game() {
            ...
            if (field == null) {
                try {
                    field = TextButtonStyle().apply {
                        ...
                        val fileHandle: FileHandle = Gdx.files.local("OpenSans.ttf")
                        font = getLabelStyle().font
   ...
   private val fontParameters = FreeTypeFontParameter().apply {
        size = 48
        color = Color.WHITE
        borderWidth = 2f
    ...
    fun getLabelStyle(): LabelStyle {
        val fileHandle: FileHandle = Gdx.files.local("assets/OpenSans.ttf")
        return getLabelStyle(fileHandle, fontParameters)
    }
    fun getLabelStyle(path: String): LabelStyle {
        val fileHandle: FileHandle = Gdx.files.local(path)
